...
try {
    ...
    if ($action === 'create' && $_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        try {
            $stmt = $pdo->prepare('INSERT INTO pedidos (user_id, estado, total) VALUES (:uid, :st, :tot)');
            $stmt->execute([':uid'=>$uid, ':st'=>'nuevo', ':tot'=>$snap['total']]);
            $orderId = (int)$pdo->lastInsertId();

            $ins = $pdo->prepare('INSERT INTO pedido_items (pedido_id, producto_id, titulo, variante, precio, cantidad) VALUES (:pid,:pr,:ti,:va,:pe,:ca)');
            foreach ($snap['items'] as $it) {
                ...
                $ins->execute([':pid'=>$orderId, ':pr'=>$prodId, ':ti'=>$titulo, ':va'=>$var, ':pe'=>$precio, ':ca'=>$cant]);
            }
    ...
    if ($action === 'list') {
        ...
        $q = $pdo->prepare('SELECT id, user_id, estado, total, creado FROM pedidos WHERE user_id=:uid ORDER BY id DESC');
        $q->execute([':uid'=>$uid]);
        ...
        $qi = $pdo->prepare("SELECT id, pedido_id, producto_id, titulo, variante, precio, cantidad FROM pedido_items WHERE pedido_id IN ($inIds) ORDER BY id");
        $qi->execute($ids);
    ...
    if ($action === 'detail') {
        ...
        $uid = (int)$_SESSION['uid'];
        $id = (int)($_GET['id'] ?? 0);
        $q = $pdo->prepare('SELECT id, user_id, estado, total, creado FROM pedidos WHERE id=:id AND user_id=:uid');
        ...
        $qi = $pdo->prepare('SELECT id, pedido_id, producto_id, titulo, variante, precio, cantidad FROM pedido_items WHERE pedido_id=:pid');
        $qi->execute([':pid'=>$id]);
